/**
 * Resume Builder Application - Complete TypeScript Implementation
 * Single file to avoid module import issues
 */
...
class NotificationSystem {
  ...
  public static show(message: string, type: NotificationType = 'info'): void {
...
class FileHandler {
  public static saveResumeToFile(data: ResumeData): void {
    try {
  ...
  public static loadResumeFromFile(
  ...
  private static generateFilename(fullName: string): string {
  ...
  private static isValidResumeData(data: any): data is ResumeData {
    ...
    if (!Array.isArray(data.work) || !Array.isArray(data.education)) return false;
...
class PDFExporter {
  public static exportToPDF(data: ResumeData, previewElement: HTMLElement): void {
    // Show options dialog
    this.showExportDialog(data, previewElement);
  }

  private static showExportDialog(data: ResumeData, previewElement: HTMLElement): void {
    // Create modal dialog
    ...
    // Event listeners
    const downloadBtn = dialog.querySelector('#downloadPdfBtn') as HTMLButtonElement;
    const printBtn = dialog.querySelector('#printPdfBtn') as HTMLButtonElement;
    const cancelBtn = dialog.querySelector('#cancelExportBtn') as HTMLButtonElement;
  ...
  private static generateAndDownloadPDF(data: ResumeData, previewElement: HTMLElement): void {
    try {
      ...
      const downloadLink = document.createElement('a');
      downloadLink.href = url;
      downloadLink.download = filename.replace('.pdf', '.html'); // Browser will handle as HTML
      downloadLink.style.display = 'none';
  ...
  private static openPrintDialog(data: ResumeData, previewElement: HTMLElement): void {
  ...
  private static generatePDFHTML(data: ResumeData, previewHTML: string): string {
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
  ...
  private static generatePDFFilename(fullName: string): string {
    const date = new Date().toISOString().split('T')[0];
    const sanitizedName = fullName
      .toLowerCase()
      .replace(/[^a-z0-9]/g, '-')
...
class FormManager {
  ...
  private workIndex: number = 0;
  private educationIndex: number = 0;

  constructor(elements: FormElements) {
    this.elements = elements;
    this.setupGlobalHandlers();
  }

  private setupGlobalHandlers(): void {
    (window as any).removeItem = (button: HTMLButtonElement) => {
      button.parentElement?.remove();
      document.dispatchEvent(new CustomEvent('formDataChanged'));
    };
  }

  public getFormData(): ResumeData {
  ...
  private collectWorkExperience(): WorkItem[] {
    const workItems: WorkItem[] = [];
    const workElements = this.elements.workContainer.querySelectorAll('.dynamic-item');

    workElements.forEach(item => {
      const company = (item.querySelector('input[name="workCompany"]') as HTMLInputElement)?.value?.trim() || '';
      const role = (item.querySelector('input[name="workRole"]') as HTMLInputElement)?.value?.trim() || '';
      const start = (item.querySelector('input[name="workStart"]') as HTMLInputElement)?.value?.trim() || '';
      const end = (item.querySelector('input[name="workEnd"]') as HTMLInputElement)?.value?.trim() || '';
      const description = (item.querySelector('textarea[name="workDescription"]') as HTMLTextAreaElement)?.value?.trim() || '';
  ...
  private collectEducation(): EducationItem[] {
    ...
    educationElements.forEach(item => {
      const school = (item.querySelector('input[name="educationSchool"]') as HTMLInputElement)?.value?.trim() || '';
      const degree = (item.querySelector('input[name="educationDegree"]') as HTMLInputElement)?.value?.trim() || '';
      const start = (item.querySelector('input[name="educationStart"]') as HTMLInputElement)?.value?.trim() || '';
      const end = (item.querySelector('input[name="educationEnd"]') as HTMLInputElement)?.value?.trim() || '';
  ...
  public loadFormData(data: ResumeData): void {
  ...
  private createWorkExperienceElement(): HTMLDivElement {
  ...
  private createEducationElement(): HTMLDivElement {
  ...
  private populateLastWorkItem(work: WorkItem): void {
    const workItems = this.elements.workContainer.querySelectorAll('.dynamic-item');
    const lastItem = workItems[workItems.length - 1];
    
    if (lastItem) {
      (lastItem.querySelector('input[name="workCompany"]') as HTMLInputElement).value = work.company || '';
      (lastItem.querySelector('input[name="workRole"]') as HTMLInputElement).value = work.role || '';
      (lastItem.querySelector('input[name="workStart"]') as HTMLInputElement).value = work.start || '';
      (lastItem.querySelector('input[name="workEnd"]') as HTMLInputElement).value = work.end || '';
      (lastItem.querySelector('textarea[name="workDescription"]') as HTMLTextAreaElement).value = work.description || '';
    }
  }

  private populateLastEducationItem(edu: EducationItem): void {
    const eduItems = this.elements.educationContainer.querySelectorAll('.dynamic-item');
    const lastItem = eduItems[eduItems.length - 1];
    
    if (lastItem) {
      (lastItem.querySelector('input[name="educationSchool"]') as HTMLInputElement).value = edu.school || '';
      (lastItem.querySelector('input[name="educationDegree"]') as HTMLInputElement).value = edu.degree || '';
      (lastItem.querySelector('input[name="educationStart"]') as HTMLInputElement).value = edu.start || '';
      (lastItem.querySelector('input[name="educationEnd"]') as HTMLInputElement).value = edu.end || '';
    }
  }
}
...
class PreviewRenderer {
  ...
  public render(data: ResumeData): void {
    if (!this.container) {
  ...
  private isEmpty(data: ResumeData): boolean {
  ...
  private renderHeader(data: ResumeData): string {
  ...
  private renderContactInfo(data: ResumeData): string {
  ...
  private renderWorkExperience(workItems: WorkItem[]): string {
  ...
  private renderWorkItem(work: WorkItem): string {
  ...
  private renderEducation(educationItems: EducationItem[]): string {
  ...
  private renderEducationItem(edu: EducationItem): string {
...
class ResumeBuilder {
  ...
  private getFormElements(): FormElements {
    ...
    return {
      fullName: getElementById<HTMLInputElement>('fullName'),
      title: getElementById<HTMLInputElement>('title'),
      email: getElementById<HTMLInputElement>('email'),
      phone: getElementById<HTMLInputElement>('phone'),
      summary: getElementById<HTMLTextAreaElement>('summary'),
  ...
  private handleLoad(): void {
    FileHandler.loadResumeFromFile(
      (data: ResumeData) => {
        try {
  ...
  public getResumeData(): ResumeData {
    return this.formManager.getFormData();
  }

  public loadResumeData(data: ResumeData): void {
    this.formManager.loadFormData(data);
    this.updatePreview();
  }
}
...
(window as any).__RESUME_BUILDER_VERSION__ = '1.0.0';